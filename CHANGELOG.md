...
### Fixed
- **Profile Registration**: Added dynamic calculation of `age` from `dateOfBirth` on the frontend before submission to resolve backend validation failures.
- **Validation Errors**: Updated API interceptors to extract and display specific Express-validator error messages instead of generic "Validation failed" alerts.

## [3.4.2] - 2026-05-31

### Changed
- **Upload Limits**: Increased maximum file size limit for image uploads from 5MB to 20MB in both backend (`uploadMiddleware.js`) and frontend validation (`CreateProfile.jsx`, `ProfileDashboard.jsx`) to resolve "File too large" errors during registration for high-quality photos.

## [3.4.1] - 2026-04-26

### Added
- **Global Admin Shortcuts**: Integrated a "New User" quick-action link in the main navigation header for administrators.
- **Enhanced Admin Navigation**: Added a persistent "New User" button in the Admin Dashboard tab list and a prominent "New" button next to the dashboard title.
- **Admin Contextual Actions**: Implemented an "Edit User" button on the public Profile Detail page (exclusive to administrators) for rapid moderation.

### Fixed
- **UX Discoverability**: Resolved an issue where the "Add New User" action was difficult to find when browsing different dashboard tabs.
