/**
 * @fileoverview SubhaLagna v3.0.1 — Real-Time Premium Chat
...
const Chat = () => {
  ...
  return (
    <div className="flex h-[80vh] bg-white/60 backdrop-blur-2xl rounded-[3rem] border border-rose-100 shadow-2xl shadow-rose-200/20 overflow-hidden animate-fade-in relative z-10">
      ...
      <aside
        className={`w-full md:w-96 flex flex-col border-r border-rose-50 bg-white/40 ${isSidebarOpen ? 'flex' : 'hidden md:flex'}`}
      >
        <div className="p-8 border-b border-rose-50 flex items-center justify-between">
          ...
          <div
            className={`w-3 h-3 rounded-full ${isConnected ? 'bg-emerald-500 shadow-lg shadow-emerald-200' : 'bg-gray-300'}`}
                ...
                >
                  ...
                  <div className="flex-1 text-left min-w-0">
                    <div className="flex justify-between items-center mb-1">
                      <p
                        className={`font-bold truncate ${isActive ? 'text-white' : 'text-gray-800'}`}
                      >
                        {partner.name}
                      </p>
                      {conv.unreadCount > 0 && !isActive && (
                        <span className="w-5 h-5 bg-rose-500 text-white text-[10px] flex items-center justify-center rounded-full border-2 border-white">
                          {conv.unreadCount}
                        </span>
                      )}
                    </div>
                    <p
                      className={`text-xs truncate ${isActive ? 'text-rose-100' : 'text-gray-400'}`}
                    >
      ...
      {/* ── Main Chat Thread ── */}
      <section
        className={`flex-1 flex flex-col bg-slate-50/30 ${isSidebarOpen ? 'hidden md:flex' : 'flex'}`}
      >
        {activeConversation ? (
          <>
            {/* Thread Header */}
            <header className="p-6 bg-white/70 backdrop-blur-md border-b border-rose-50 flex items-center gap-4">
              <button
                ...
                className="md:hidden p-2 text-rose-500 hover:bg-rose-50 rounded-xl"
              >
              ...
              <img
                src={getPartnerData(activeConversation).photo}
                className="w-12 h-12 rounded-2xl object-cover"
                alt=""
              />
              <div>
                <h3 className="font-bold text-gray-800">
                  {getPartnerData(activeConversation).name}
                </h3>
                {typingUser ? (
              ...
              {loadingMessages ? (
                <div className="space-y-4">
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className={`h-12 w-48 rounded-2xl animate-pulse ${i % 2 === 0 ? 'bg-gray-100 self-end' : 'bg-gray-200'}`}
                    />
                  ))}
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {msg.content}
                        <div
                          className={`text-[9px] mt-1 text-right opacity-60 ${isOwn ? 'text-white' : 'text-gray-400'}`}
                        >
