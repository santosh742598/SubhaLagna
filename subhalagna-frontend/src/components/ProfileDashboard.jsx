...
const DashboardCard = ({ children, title, icon, delay = "0", onSave, isSaving, loadingText = "Saving..." }) => (
  <div 
...
const ProfileDashboard = () => {
  ...
  const handleSaveSection = async (sectionFields, extraData = {}) => {
    ...
    if (needsFormData) {
      ...
      if (extraData.removePhotos) submitData.append('removePhotos', JSON.stringify(extraData.removePhotos));
  ...
  if (!user?.profile) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-rose-50/30">
        ...
        <p className="text-rose-500 font-bold font-serif text-xl animate-pulse">Designing your universe...</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          {(() => {
            ...
            return (
              <div className={`bg-gradient-to-br ${s.bg} rounded-[2.5rem] p-8 text-white shadow-xl ${s.glow} relative overflow-hidden group border ${s.border}`}>
                ...
                <div className="relative z-10">
                   <div className="flex items-center gap-3 mb-6">
                     ...
                     <div>
                       <h3 className={`font-bold text-lg leading-tight ${s.text}`}>{user.isPremium ? `${user.premiumPlan.toUpperCase()} MEMBER` : 'FREE PLAN'}</h3>
                   ...
                   {user.isPremium ? (
                     <div className="space-y-4">
                           ...
                           <>
                             <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                                ...
                                <div className="w-full h-1 bg-white/10 rounded-full mt-2 overflow-hidden">
                                   <div className={`h-full ${s.badge} transition-all duration-1000`} style={{ width: `${Math.max(0, ((30 - (user.contactsViewed?.length || 0)) / 30) * 100)}%` }} />
                                </div>
                             </div>
                        ...
                        {user.premiumPlan === 'platinum' && (
                           <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] block mb-1">Status</span>
           ...
           {activeTab === 'profile' ? (
               <div className="relative z-10">
                 ...
                 <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 p-8 bg-slate-900 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                       <h1 className="text-3xl md:text-4xl font-serif font-black text-white mb-2 leading-tight">Master Profile <span className="text-rose-500">Editor</span></h1>
                       <p className="text-slate-400 font-medium text-sm max-w-md">Edit sections individually for faster updates. Your profile is <span className="text-emerald-400 font-bold">{profileStrength}% complete</span>.</p>
                    </div>
                    <button 
                      type="button"
                 ...
                 <DashboardCard 
                    ...
                    onSave={() => handleSaveSection(['height', 'motherTongue', 'religion', 'caste', 'bio', 'dateOfBirth'], { profilePhoto: file, hasFiles: !!file })}
                      ...
                      <div className="lg:col-span-5 border-r border-rose-50/60 pr-0 lg:pr-10">
                        <div className="bg-slate-50/50 p-6 rounded-[2.5rem] border border-slate-100 flex flex-col items-center">
                          ...
                          <div className="flex gap-3 w-full">
                            ...
                            <button type="button" onClick={() => setIsCameraOpen(true)} className="flex-1 py-3 bg-rose-600 text-white rounded-xl font-bold text-xs shadow-lg shadow-rose-100 hover:bg-rose-700 transition-all">
                               Camera
                            </button>
                          </div>
                        </div>
                      ...
                      <div className="lg:col-span-7 space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                           <div className="opacity-60 grayscale-[0.5]">
                              ...
                              <input type="text" value={formData.name} disabled className="w-full px-5 py-3.5 rounded-2xl border border-slate-100 bg-slate-50/50 text-sm font-bold cursor-not-allowed" />
                           </div>
                           <div className="opacity-60 grayscale-[0.5]">
                              <FormLabel>GENDER</FormLabel>
                              <input type="text" value={formData.gender} disabled className="w-full px-5 py-3.5 rounded-2xl border border-slate-100 bg-slate-50/50 text-sm font-bold cursor-not-allowed" />
                           </div>
                        </div>
                        ...
                        {(() => {
                            ...
                            return (
                              <div>
                                <FormLabel>
                                   DATE OF BIRTH
                                   {hasDOB && <span className="ml-2 text-[8px] px-2 py-0.5 bg-slate-100 text-slate-400 rounded-md">VERIFIED</span>}
                                </FormLabel>
                                {hasDOB ? (
                                  <input type="text" value={formatDate(formData.dateOfBirth)} disabled className="w-full px-5 py-3.5 rounded-2xl border border-slate-100 bg-slate-50/50 text-sm font-bold opacity-60 cursor-not-allowed" />
                                ) : (
                                  <input type="date" name="dateOfBirth" value={toInputDate(formData.dateOfBirth)} onChange={handleChange} className="w-full px-5 py-3.5 rounded-2xl border border-rose-100 bg-rose-50/10 text-sm font-bold focus:ring-4 focus:ring-rose-500/10 outline-none transition-all" />
                                )}
                              </div>
                            );
                        })()}
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-rose-50/50">
                           <div>
                              ...
                              <input type="text" name="height" value={formData.height} onChange={handleChange} placeholder={"5'8\""} className="w-full px-5 py-3.5 rounded-2xl border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white transition-all" />
                           </div>
                           <SearchableDropdown label="MOTHER TONGUE" name="motherTongue" value={formData.motherTongue} options={languageOptions} onChange={handleChange} />
                        </div>

                        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-rose-50/50">
                           <SearchableDropdown label="RELIGION" name="religion" value={formData.religion} options={religionOptions} onChange={handleChange} />
                           <SearchableDropdown label="CASTE" name="caste" value={formData.caste} options={casteOptions} onChange={handleChange} />
                        </div>
                      </div>
                   </div>
                   <div className="mt-8">
                     <FormLabel>MY BIO / ABOUT ME</FormLabel>
                     <textarea name="bio" value={formData.bio} onChange={handleChange} rows="3" placeholder="Tell potential matches about your values, goals and what you're looking for..." className="w-full px-6 py-4 rounded-[1.5rem] border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white focus:ring-4 focus:ring-rose-500/5 transition-all outline-none resize-none" />
                   </div>
                 </DashboardCard>
                 ...
                 >
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div>
                         ...
                         <input type="text" name="education" value={formData.education} onChange={handleChange} placeholder="e.g. Masters in Data Science" className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white transition-all outline-none" />
                      </div>
                      <div>
                         <FormLabel>PROFESSION / ROLE</FormLabel>
                         <input type="text" name="profession" value={formData.profession} onChange={handleChange} placeholder="e.g. Senior Architect" className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white transition-all outline-none" />
                      </div>
                   </div>
                   <div className="mt-6 pt-6 border-t border-slate-50">
                      <FormLabel>CURRENT RESIDENCE (CITY)</FormLabel>
                      <input type="text" name="location" value={formData.location} onChange={handleChange} placeholder="Search city..." className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white transition-all outline-none" />
                   </div>
                 </DashboardCard>
                 ...
                 >
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div>
                         ...
                         <input type="text" name="fatherName" value={formData.fatherName} onChange={handleChange} className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white transition-all outline-none" />
                      </div>
                      <div>
                         <FormLabel>MOTHER'S FULL NAME</FormLabel>
                         <input type="text" name="motherName" value={formData.motherName} onChange={handleChange} className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white transition-all outline-none" />
                      </div>
                   </div>
                   <div className="grid grid-cols-2 gap-8 mt-8">
                      <div>
                         <FormLabel>SIBLINGS</FormLabel>
                         <input type="number" name="siblings" value={formData.siblings} onChange={handleChange} className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white transition-all outline-none" />
                      </div>
                      <div>
                         <FormLabel>FAMILY TYPE</FormLabel>
                         <select name="familyType" value={formData.familyType} onChange={handleChange} className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white transition-all outline-none cursor-pointer">
                 ...
                 >
                   <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                      ...
                      <div className="md:col-span-2">
                        ...
                        <select name="nakshatra" value={formData.nakshatra} onChange={handleChange} className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white outline-none cursor-pointer transition-all">
                      ...
                      <div>
                        ...
                        <select name="pada" value={formData.pada} onChange={handleChange} className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white outline-none cursor-pointer transition-all">
                          <option value="">Select Pada</option>
                          {[1,2,3,4].map(p => <option key={p} value={p}>{p}st Pada</option>)}
                        </select>
                      </div>
                      <div className="md:col-span-2">
                 ...
                 <DashboardCard 
                    ...
                    onSave={() => handleSaveSection([], { traits, interests: interestsList, hasFiles: true })}
                    isSaving={isSubmitting}
                 >
                   ...
                   <div className="space-y-8">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        <div className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100">
                          ...
                          <div className="flex items-center gap-4 mt-2">
                             <div className="flex-1">
                                <span className="text-[9px] text-slate-400 font-bold block mb-1">MIN</span>
                                <input type="number" name="prefMinAge" value={formData.prefMinAge} onChange={handleChange} className="w-full px-4 py-3 rounded-xl border border-white bg-white shadow-sm text-sm font-bold" />
                             </div>
                             <div className="pt-5 font-black text-rose-200">〰</div>
                             <div className="flex-1">
                                <span className="text-[9px] text-slate-400 font-bold block mb-1">MAX</span>
                                <input type="number" name="prefMaxAge" value={formData.prefMaxAge} onChange={handleChange} className="w-full px-4 py-3 rounded-xl border border-white bg-white shadow-sm text-sm font-bold" />
                             </div>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col justify-end">
                           ...
                           <input type="text" name="prefLocation" value={formData.prefLocation} onChange={handleChange} placeholder="e.g. Any, Delhi, Maharashtra" className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50/30 text-sm font-medium focus:bg-white outline-none" />
                        </div>
                     </div>

                     <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pt-6 border-t border-slate-50">
                        <SearchableDropdown label="Expected Religion" name="prefReligion" value={formData.prefReligion} options={['Any', ...religionOptions]} onChange={handleChange} />
                        <SearchableDropdown label="Expected Caste" name="prefCaste" value={formData.prefCaste} options={['Any', ...casteOptions]} onChange={handleChange} placeholder="e.g. Brahmin, Any" />
                     </div>
                   </div>
                 </DashboardCard>
                 ...
                 <DashboardCard 
                    ...
                    onSave={() => handleSaveSection([], { additionalPhotos: galleryFiles, removePhotos, hasFiles: true })}
                    isSaving={isSubmitting}
                 >
                    <p className="text-xs text-slate-400 mb-8 font-medium italic">Showing up to 5 additional moments from your life.</p>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
                        {existingGallery.map((photoUrl, idx) => (
                          <div key={idx} className="relative aspect-square rounded-[1.5rem] overflow-hidden shadow-md group border border-slate-100">
                        ...
                        {galleryPreviews.map((src, idx) => (
                          <div key={'new-'+idx} className="relative aspect-square rounded-[1.5rem] overflow-hidden shadow-md group border border-emerald-100 ring-2 ring-emerald-50">
                            <img src={src} className="w-full h-full object-cover" alt="" />
                            <div className="absolute top-2 left-2 bg-emerald-500 text-white text-[8px] px-2 py-0.5 rounded-md font-black">NEW</div>
                 ...
                 >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <div className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100">
                          ...
                          <select name="showPhotoTo" value={formData.showPhotoTo} onChange={handleChange} className="w-full px-4 py-3 rounded-xl border border-white bg-white shadow-sm text-sm font-bold text-slate-700 outline-none cursor-pointer">
                       ...
                       <div className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100">
                          ...
                          <select name="showContactTo" value={formData.showContactTo} onChange={handleChange} className="w-full px-4 py-3 rounded-xl border border-white bg-white shadow-sm text-sm font-bold text-slate-700 outline-none cursor-pointer">
