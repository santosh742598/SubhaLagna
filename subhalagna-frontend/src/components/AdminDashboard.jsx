/**
 * @fileoverview SubhaLagna v2.4.0 — Admin Dashboard
...
const AdminDashboard = () => {
  ...
  const [userForm, setUserForm] = useState({
    ...
    premiumExpires: '',
    profileData: {
      gender: 'Male', religion: 'Hindu', caste: '', 
      currentCity: '', currentState: '', 
      nativeCity: '', nativeState: '',
      education: 'Graduate', profession: 'Professional', height: "5' 5\"", bio: '',
      motherTongue: '',
      family: { fatherName: '', motherName: '', siblings: '0', familyType: 'Nuclear' },
      horoscope: { dateOfBirth: '', timeOfBirth: '', placeOfBirth: '', rashi: '', nakshatra: '', pada: '', gotra: '', manglik: 'Unknown' }
    }
  });
  ...
  const handleOpenAddUser = () => {
    ...
    setUserForm({
      ...
      isPremium: false, premiumPlan: 'none', premiumExpires: '',
      profileData: {
        gender: 'Male', religion: 'Hindu', caste: '', 
        currentCity: '', currentState: '', 
        nativeCity: '', nativeState: '',
  ...
  const handleOpenEditUser = (user) => {
    ...
    setUserForm({
      ...
      premiumExpires: user.premiumExpires ? new Date(user.premiumExpires).toISOString().split('T')[0] : '',
      profileData: {
        ...
        family: profile.family || { fatherName: '', motherName: '', siblings: '0', familyType: 'Nuclear' },
        horoscope: {
          ...(profile.horoscope || {}),
          dateOfBirth: profile.horoscope?.dateOfBirth ? new Date(profile.horoscope.dateOfBirth).toISOString().split('T')[0] : '',
      ...
      <main className="max-w-7xl mx-auto pt-10 pb-20">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex gap-2 flex-wrap">
             ...
             <button onClick={() => setActiveTab('transactions')} className={`px-4 py-2 rounded-xl border text-xs font-bold transition-all ${activeTab === 'transactions' ? 'bg-rose-600 text-white border-rose-600 shadow-lg shadow-rose-100' : 'bg-white text-gray-400 border-gray-100'}`}>Transactions</button>
          </div>
        </div>
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4">
                ...
                <button onClick={handleOpenAddUser} className="bg-rose-600 text-white px-6 py-2 rounded-xl text-xs font-bold shadow-lg shadow-rose-100 hover:scale-105 transition-all flex items-center gap-2">
                    ...
                    users.map((user) => (
                      <tr key={user._id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-100 shadow-sm">
                               <img 
                                 ...
                                 alt=""
                                 onError={(e) => { e.target.src = '/man.png'; }}
                               />
                            </div>
                            <div>
                        ...
                        <td className="px-8 py-5 text-right flex items-center justify-end gap-2">
                             <button onClick={() => handleOpenEditUser(user)} className="p-2 rounded-xl border border-rose-100 bg-rose-50 text-rose-600 hover:bg-rose-100 transition-all" title="Edit Account & Profile">
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden animate-fade-in">
             <div className="p-8 border-b border-gray-50 flex justify-between items-center">
                <div>
                   ...
                   <p className="text-gray-400 text-xs mt-1">Full history of platform revenue and membership activations.</p>
                </div>
                <div className="flex items-center gap-3">
                    <div className="flex flex-col items-end">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Platform Revenue</span>
                      <span className="text-xl font-black text-emerald-600">₹{(stats?.totalRevenue || 0).toLocaleString()}</span>
                    </div>
                </div>
             </div>
             <div className="overflow-x-auto">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-gray-50 text-sm">
                    {transactions.map((tx) => (
                      <tr key={tx._id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-8 py-5">
                          ...
                          <p className="text-[10px] text-gray-400 uppercase tracking-tighter">{new Date(tx.createdAt).toLocaleTimeString()}</p>
                        </td>
                        <td className="px-6 py-5">
                        ...
                        <td className="px-6 py-5">
                          <p className="font-black text-slate-700 uppercase tracking-tight">{tx.planId}</p>
                          <p className="font-bold text-emerald-600">₹{tx.amount}</p>
                        </td>
                        <td className="px-6 py-5">
                          <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-widest ${tx.type === 'razorpay' ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-600'}`}>
                            {tx.type}
                          </span>
                        </td>
                        <td className="px-6 py-5">
                          <span className={`flex items-center gap-1.5 text-xs font-bold ${tx.status === 'captured' ? 'text-emerald-500' : tx.status === 'pending' ? 'text-amber-500' : 'text-slate-400'}`}>
                             <span className={`w-1.5 h-1.5 rounded-full ${tx.status === 'captured' ? 'bg-emerald-500' : tx.status === 'pending' ? 'bg-amber-500' : 'bg-slate-300'}`} />
                    ...
                    {transactions.length === 0 && (
                      <tr><td colSpan="5" className="p-20 text-center text-gray-300 italic">No transactions recorded yet.</td></tr>
                    )}
                  </tbody>
                </table>
             </div>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
            <div className="bg-white rounded-[3rem] w-full max-w-4xl max-h-[90vh] overflow-y-auto p-10 shadow-2xl animate-scale-in">
               <div className="flex justify-between items-center mb-8">
                  <div>
                     <h3 className="text-2xl font-serif font-bold text-gray-800">{userFormMode === 'add' ? 'Add New User' : 'Edit User Info'}</h3>
                     <p className="text-gray-400 text-sm italic">Full control over account and profile data.</p>
                  </div>
                  <button onClick={() => setShowUserModal(false)} className="text-gray-400 hover:text-gray-600"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" /></svg></button>
               </div>
               ...
               <form onSubmit={handleSaveUser} className="space-y-8">
                  ...
                  {userModalTab === 'account' && (
                    <div className="space-y-8 animate-fade-in">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         <div className="col-span-full pb-2 border-b border-gray-50"><h4 className="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em]">Account Credentials</h4></div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Display Name</label>
                            <input type="text" name="name" required value={userForm.name} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-rose-500 font-medium" />
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Email Address</label>
                            <input type="email" name="email" required value={userForm.email} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-rose-500 font-medium" />
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Password {userFormMode === 'edit' && '(Blank to keep same)'}</label>
                            <input type="password" name="password" required={userFormMode === 'add'} value={userForm.password} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-rose-500 font-medium" />
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">System Role</label>
                            <select name="role" value={userForm.role} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm font-bold outline-none focus:ring-2 focus:ring-rose-500">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                         <div className="col-span-full pb-2 border-b border-gray-50"><h4 className="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em]">Membership Status</h4></div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Is Premium?</label>
                            <select name="isPremium" value={userForm.isPremium} onChange={e => handleUserFormChange({ target: { name: 'isPremium', value: e.target.value === 'true' } })} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm font-bold">
                               <option value="false">No</option>
                               <option value="true">Yes</option>
                            </select>
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Current Plan</label>
                            <select name="premiumPlan" disabled={!userForm.isPremium} value={userForm.premiumPlan} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm font-bold disabled:opacity-50">
                         ...
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Expiry Date</label>
                            <input type="date" name="premiumExpires" disabled={!userForm.isPremium} value={userForm.premiumExpires} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm disabled:opacity-50" />
                         </div>
                      </div>
                    </div>
                  )}
                  ...
                  {userModalTab === 'profile' && (
                    <div className="space-y-8 animate-fade-in">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         <div className="col-span-full pb-2 border-b border-gray-50"><h4 className="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em]">Background & Bio</h4></div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Gender</label>
                            <select name="profile.gender" value={userForm.profileData.gender} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold">
                         ...
                         <div>
                            ...
                            <input type="date" name="horoscope.dateOfBirth" value={userForm.profileData.horoscope?.dateOfBirth} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm" />
                         </div>
                         <div>
                            <SearchableDropdown label="Religion" name="profile.religion" value={userForm.profileData.religion} options={religionOptions} onChange={handleUserFormChange} />
                         </div>
                         <div>
                            <SearchableDropdown label="Caste / Community" name="profile.caste" value={userForm.profileData.caste} options={casteOptions} onChange={handleUserFormChange} placeholder="e.g. Brahmin, Rajput" />
                         </div>
                         <div>
                            <SearchableDropdown label="Mother Tongue" name="profile.motherTongue" value={userForm.profileData.motherTongue} options={languageOptions} onChange={handleUserFormChange} placeholder="e.g. Hindi, English" />
                         </div>
                         <div className="md:col-span-3">
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Short Bio</label>
                            <textarea name="profile.bio" rows="3" value={userForm.profileData.bio} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm outline-none no-scrollbar resize-none" placeholder="User story, personality traits..." />
                         </div>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         <div className="col-span-full pb-2 border-b border-gray-50"><h4 className="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em]">Physical & Professional</h4></div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Height (e.g. 5' 11")</label>
                            <input type="text" name="profile.height" value={userForm.profileData.height} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm" />
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Education</label>
                            <input type="text" name="profile.education" value={userForm.profileData.education} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm" />
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Profession</label>
                            <input type="text" name="profile.profession" value={userForm.profileData.profession} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm" />
                         </div>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         <div className="col-span-full pb-2 border-b border-gray-50"><h4 className="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em]">Residence & Native</h4></div>
                         <div>
                            <SearchableDropdown label="Current State" name="profile.currentState" value={userForm.profileData.currentState} options={stateOptions} onChange={handleUserFormChange} placeholder="Select state..." />
                         </div>
                         <div>
                            <SearchableDropdown label="Current City" name="profile.currentCity" value={userForm.profileData.currentCity} options={curCityOptions} onChange={handleUserFormChange} placeholder="Select city..." disabled={!userForm.profileData.currentState} minChars={2} />
                         </div>
                         <div>
                            <SearchableDropdown label="Native State" name="profile.nativeState" value={userForm.profileData.nativeState} options={stateOptions} onChange={handleUserFormChange} placeholder="Select state..." />
                         </div>
                         <div>
                            <SearchableDropdown label="Native City" name="profile.nativeCity" value={userForm.profileData.nativeCity} options={natCityOptions} onChange={handleUserFormChange} placeholder="Select city..." disabled={!userForm.profileData.nativeState} minChars={2} />
                         </div>
                      </div>
                    </div>
                  )}
                  ...
                  {userModalTab === 'family' && (
                    <div className="space-y-8 animate-fade-in">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         <div className="col-span-full pb-2 border-b border-gray-50"><h4 className="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em]">Family Background</h4></div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Father's Name</label>
                            <input type="text" name="family.fatherName" value={userForm.profileData.family?.fatherName} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm" />
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Mother's Name</label>
                            <input type="text" name="family.motherName" value={userForm.profileData.family?.motherName} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm" />
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Siblings</label>
                            <input type="text" name="family.siblings" value={userForm.profileData.family?.siblings} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm" />
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Family Type</label>
                            <select name="family.familyType" value={userForm.profileData.family?.familyType} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold">
                  ...
                  {userModalTab === 'horoscope' && (
                    <div className="space-y-8 animate-fade-in">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         <div className="col-span-full pb-2 border-b border-gray-50"><h4 className="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em]">Astrological Data</h4></div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Time of Birth (e.g. 10:30 AM)</label>
                            <input type="text" name="horoscope.timeOfBirth" value={userForm.profileData.horoscope?.timeOfBirth} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm" />
                         </div>
                         <div className="md:col-span-2">
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Place of Birth</label>
                            <input type="text" name="horoscope.placeOfBirth" value={userForm.profileData.horoscope?.placeOfBirth} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm" />
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Nakshatra</label>
                            <select name="horoscope.nakshatra" value={userForm.profileData.horoscope?.nakshatra} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold">
                               <option value="">Select Nakshatra</option>
                               {NAKSHATRAS.map(n => <option key={n} value={n}>{n}</option>)}
                            </select>
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Pada</label>
                            <select name="horoscope.pada" value={userForm.profileData.horoscope?.pada} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold">
                               <option value="">Select Pada</option>
                               {PADAS.map(p => <option key={p} value={p}>{p}st Pada</option>)}
                            </select>
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Rashi (Moon Sign)</label>
                            <select 
                              name="horoscope.rashi" 
                              value={userForm.profileData.horoscope?.rashi} 
                              onChange={handleUserFormChange} 
                              disabled={!!(userForm.profileData.horoscope?.nakshatra && userForm.profileData.horoscope?.pada)}
                         ...
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Gotra</label>
                            <input type="text" name="horoscope.gotra" value={userForm.profileData.horoscope?.gotra} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm" />
                         </div>
                         <div>
                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Is Manglik?</label>
                            <select name="horoscope.manglik" value={userForm.profileData.horoscope?.manglik} onChange={handleUserFormChange} className="w-full bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold">
                  ...
                  {userModalTab === 'media' && (
                    <div className="space-y-8 animate-fade-in">
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          <div>
                             <h4 className="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em] mb-4">Main Profile Photo</h4>
                             <div className="relative group w-40 h-40 rounded-3xl overflow-hidden border-2 border-dashed border-gray-200 hover:border-rose-400 transition-all">
                                {photoFiles.profilePhoto ? (
                                   <img src={URL.createObjectURL(photoFiles.profilePhoto)} className="w-full h-full object-cover" alt="Preview" />
                                ) : (
                                   <img src={userForm.profileData.profilePhoto} className="w-full h-full object-cover opacity-50" alt="Current" />
                                )}
                                <input type="file" accept="image/*" onChange={e => setPhotoFiles({...photoFiles, profilePhoto: e.target.files[0]})} className="absolute inset-0 opacity-0 cursor-pointer" />
                          ...
                          <div className="space-y-4">
                             <h4 className="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em]">Add Gallery Photos (Max 5)</h4>
                             <label className="block w-full bg-slate-50 border border-gray-100 rounded-xl p-6 text-center cursor-pointer hover:bg-slate-100 transition-all">
                                <span className="text-gray-400 text-[10px] font-bold uppercase tracking-widest">Click to Select Multiple Images</span>
                                <input type="file" multiple accept="image/*" onChange={e => setPhotoFiles({...photoFiles, additionalPhotos: [...photoFiles.additionalPhotos, ...Array.from(e.target.files)]})} className="hidden" />
                             </label>
                             <div className="flex flex-wrap gap-2">
                                {photoFiles.additionalPhotos.map((file, idx) => (
                                   <div key={idx} className="relative w-16 h-16 rounded-xl overflow-hidden border border-gray-200">
                                      ...
                                      <button type="button" onClick={() => setPhotoFiles({...photoFiles, additionalPhotos: photoFiles.additionalPhotos.filter((_, i) => i !== idx)})} className="absolute top-1 right-1 bg-white/80 rounded-full p-0.5"><svg className="w-3 h-3 text-rose-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" /></svg></button>
                                   </div>
                                ))}
                             </div>
                          </div>
                       ...
                       {userFormMode === 'edit' && targetUserId && (
                          <div className="pt-6 border-t border-gray-50 flex justify-center">
                             <button 
                               type="button" 
                               disabled={isUploading || (!photoFiles.profilePhoto && photoFiles.additionalPhotos.length === 0)}
                               onClick={() => handleUploadPhotos(users.find(u => u._id === targetUserId)?.profile?._id)}
                  ...
                  <div className="pt-6 border-t border-gray-50 flex justify-end gap-3">
                     <button type="button" onClick={() => setShowUserModal(false)} className="px-6 py-2.5 bg-white border border-gray-100 rounded-xl text-xs font-bold text-gray-400 hover:text-gray-600">Cancel</button>
                     <button type="submit" className="px-10 py-2.5 bg-rose-600 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-rose-100 hover:-translate-y-1 transition-all">
