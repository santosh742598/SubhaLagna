...
/**
 * @fileoverview SubhaLagna v2.0.0 — Global Header
...
const Header = () => {
  ...
  const navLinks = [
    { name: 'Matches', path: '/matches', icon: 'M12 4.318l-1.318 1.318L12 6.954l1.318-1.318L12 4.318zM12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z' },
    ...
    <header className="sticky top-0 w-full z-50 bg-white/80 backdrop-blur-lg border-b border-pink-100/60 shadow-sm transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
            ...
            <nav className="hidden md:flex items-center gap-1">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="relative group">
                  ...
                  >
                    <img 
                      src={user?.profile?.profilePhoto || user?.profile?.image || '/placeholder-profile.png'} 
                      alt="Profile"
          ...
          {!token && (
             <div className="flex items-center gap-4">
                <Link to="/login" className="text-sm font-bold text-slate-600 hover:text-pink-600 transition-colors px-4">Login</Link>
                <Link to="/register" className="px-5 py-2.5 rounded-xl bg-pink-500 text-white text-sm font-bold hover:bg-pink-600 transition-all shadow-lg shadow-pink-200">
                  Join Free
                </Link>
             </div>
          )}
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`flex items-center gap-4 p-4 rounded-xl text-base font-bold transition-colors ${
                  location.pathname === link.path ? 'bg-pink-50 text-pink-600' : 'text-slate-600 hover:bg-slate-50'
                }`}
              >
